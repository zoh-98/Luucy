...
export async function all(m, chatUpdate) {
  ...
  for (const name in global.plugins) {
    ...
    if ((usedPrefix = "")) {
      ...
      const isId = plugin.config.name instanceof RegExp ? // RegExp Mode?
                plugin.config.name.test(command) :
                Array.isArray(plugin.config.name) ? // Array?
                    plugin.config.name.some((cmd) => cmd instanceof RegExp ? // RegExp in Array?
                        cmd.test(command) :
                        cmd === command,
                    ) :
                    typeof plugin.config.name === 'string' ? // String?
