...
class CustomError extends Error {
 constructor(obj) {
  ...
  if (typeof obj !== 'object' || obj === null)
   throw new TypeError('Object importd');
...
function createOraDots(text) {
 ...
 spin._start = () => {
  funcs.enableStderrClearLine(false);
  spin.start();
 };
 spin._stop = () => {
  funcs.enableStderrClearLine(true);
  spin.stop();
 };
 return spin;
}
...
function message(api, event) {
 async function sendMessageError(err) {
  if (typeof err === "object" && !err.stack)
   err = funcs.removeHomeDir(JSON.stringify(err, null, 2));
  else
   err = funcs.removeHomeDir(`${err.name || err.error}: ${err.message}`);
  return await api.sendMessage(funcs.getText("funcs", "errorOccurred", err), event.threadID, event.messageID);
 }
 return {
...
const funcs = {
 ...
 isNumber,
 jsonStringifyColor,
 loading: import("./logger/loading.js"),
 log,
 logColor: import("./logger/logColor.js"),
 message,
 randomString,
 randomNumber,
 removeHomeDir,
